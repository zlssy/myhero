define('font', function(require, exports, module){
	var Image = require('image');
	
	GameEngine.Font = GameEngine.Image.extend({
		widthMap: [],
		indices: [],
		firstChar: 32,
		alpha: 1,
		...
		draw: function(text, x, y, align) {
			...
			if (align == GameEngine.Font.ALIGN.RIGHT || align == GameEngine.Font.ALIGN.CENTER) {
				var width = this._widthForLine(text);
				x -= align == GameEngine.Font.ALIGN.CENTER ? width / 2 : width;
			}
			if (this.alpha !== 1) {
			...
			GameEngine.Image.drawCount += text.length;
		},
		_drawChar: function(c, targetX, targetY) {
			...
			GameEngine.system.context.drawImage(this.data, charX, charY, charWidth, charHeight, GameEngine.system.getDrawPos(targetX), GameEngine.system.getDrawPos(targetY), charWidth, charHeight);
		...
		_loadMetrics: function(image) {
			...
			this.widthMap = [];
			this.indices = [];
			var px = GameEngine.getImagePixels(image, 0, image.height - 1, image.width, 1);
			var currentChar = 0;
			var currentWidth = 0;
	...
	GameEngine.Font.ALIGN = {
		LEFT: 0,
		RIGHT: 1,
		CENTER: 2
	};

	module.exports = GameEngine.Font;
});